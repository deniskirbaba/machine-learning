...
На каждом шаге дерева происходит разбиение выборки на две подвыборки на основе сравнения элементов выборки с некоторым пороговым значением определенного признака. Это происходит рекурсивно, до тех пор пока не будет достигнута максимальная глубина дерева.
...
# Bias-variance
У нас у ошибки предсказания есть bias (смещение) и variance (разброс). Смещение - насколько предсказание модели в среднем отличается от истинного значения таргета. Разброс - неустойчивость модели (насколько одна и та же модель будет давать разные результаты на выборках из одного и того же генерального распределения) (то есть малое изменение обучающей выборки приводит к значительному изменению предсказаний модели). 
...
Стоит отметить, что в среднем ошибка OOB будет выше, чем при обычном тестировании на валидационной выборке, так как суть RF в том что при увеличении количества деревьев, ошибка предсказания уменьшается, а при OOB мы уменьшаем количество деревьев, выкидывая те, которые уже видели определенные объекты при бустрапировании.
...
Также стоит помнить, что дерево - кусочно-постоянная функция, а значит дерево не способно к экстраполяции кроме константных значений. То есть значение целевой переменной за пределами выпуклой оболочки всех точеек обучающей выборки будут константами.
...
# Blending
Возьмем 5 разных моделей (например: kNN, LogReg, RF, NB, DT). Будем обучать их в задаче классификации. Каждая из моделей предсказывает вероятность по каждому классу таргета и у каждой модели сущность этой вероятности различная. 
Возьмем все эти предсказанные вероятности от каждой модели и составим взвешенную (по новым параметрам) сумму вероятностей. И найдем веса, путем обучения на заранее отложенной выборке (часть валидационной). Таким образом получим решение задачи.
По сути тут происходит преобразование данных из исходного признакового описания в новое (5 признаков - вероятности от каждой модели)