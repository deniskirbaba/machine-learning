...
def rbf(x_1, x_2, sigma=1.):
    ...
    """
    x_1_sq_norm = torch.square(x_1).sum(dim=1, keepdim=True)
    x_2_sq_norm = torch.square(x_2).sum(dim=1, keepdim=False)
    x_1_x_2_pairwise_mult = torch.matmul(x_1, x_2.T)
    distances = x_1_sq_norm + x_2_sq_norm - 2 * x_1_x_2_pairwise_mult
    rbf_distances = torch.exp(-distances / (2 * sigma**2))
    return torch.Tensor(rbf_distances).type(torch.float32)
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.clamp(1 - labels * scores, 0))
